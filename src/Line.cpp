...
{
    assert(str1_ptr and str2_ptr);

    char const *pos1 = str1_ptr->beg, *pos2 = str2_ptr->beg;
    while (pos1 != str1_ptr->end and pos2 != str2_ptr->end)
    {
        char c1 = (char)tolower((int)*pos1), c2 = (char)tolower((int)*pos2);
        if (!isalpha(c1))
        {
            ++pos1;
            continue;
    ...
    if (pos2 != str2_ptr->end)
    {
        if (pos1 != str1_ptr->end)
        {
            return 0;
        }
        else
...
int my_rev_cmp(One_line const *const str1_ptr, One_line const *const str2_ptr)
{
    ...
    char const *pos1 = str1_ptr->end - 1, *pos2 = str2_ptr->end - 1;
    ...
    if (pos2 != str2_ptr->beg - 1)
    {
        if (pos1 != str1_ptr->beg - 1)
        {
            return 0;
        }
        else
